...
fn_authors = "/opt/airflow/dags/data/util/web_scraping.txt"
fname_main_df = 'main_df.csv'
fname2_main_df = 'main_df.csv'

from variables import data_source_raw, data_source_cleaned, data_source_json, data_source_tmp, data_source_final
...
def add_authors():
    df_main = load_main(fname_main_df, data_source_json, False)
    df_main2 = load_main(fname2_main_df, data_source_tmp, True)
    ...
    # Reorder columns:
    df_main = df_main[
        ['title', 'url', 'description', 'views', 'author', 'updater', 'added', 'updated', 'year', 'has_relations',
         'has_other_texts', 'has_refs', 'has_DBpedia', 'status', 'adult', 'medical', 'racy', 'spoof', 'violence',
         'template_image_url', 'fb_image_url']]

    df_main2 = df_main2[
        ['title', 'url', 'description', 'views', 'author', 'updater', 'added', 'updated', 'year', 'has_relations',
         'has_other_texts', 'has_refs', 'has_DBpedia', 'status', 'adult', 'medical', 'racy', 'spoof', 'violence',
         'template_image_url', 'fb_image_url', 'alt_img_urls', 'similar_images', 'similar_images2',
         'image_recogn_labels', 'image_recogn_scores', 'search_keywords']]
    ...
    df_main.to_csv(data_source_final + 'main_df.csv', sep=';', index=True, encoding='utf-8')

    df_main2.to_csv(data_source_final + 'main_df.csv', sep=';', index=True, encoding='utf-8')

    # Authors and updaters:
    df_authors.to_csv(data_source_final + 'authors_df.csv', sep=';', index=True, encoding='utf-8')
    df_updaters.to_csv(data_source_final + 'updaters_df.csv', sep=';', index=True, encoding='utf-8')

    df_authors.to_json(data_source_final + 'authors.json', force_ascii=False, orient='index')
    df_updaters.to_json(data_source_final + 'updaters.json', force_ascii=False, orient='index')
