...
def cleaning_1():
    ...
    drop_values = ['fuck', 'shit', 'dick', 'Fuck', 'Shit', 'Dick', 'FUCK', 'penis', 'Penis', 'PENIS', 'Gay', 'gay',
                   'GAY', 'Naked', 'poop', 'Poop', 'Nigga', 'Nigger', 'nigga', 'Suck', 'Cock', 'Porn', 'porn', 'Ass',
                   'Raped', 'raped', 'Rape', 'Pussy', 'Cunt', 'Bitch']
    ...
    def check_if_link(key, value):
        exluded = ["facebook", "translate", "uncyclopedia", "youtu.be", "/nl.", "/es.", "/fr.", "/zh.", "/zh-", "/sv.",
                   "/da.", "/pt.", "/au.", "/hu.", "/it.", "/no.", "/ja.", "/id.", "/fi.", "/de.", "/cs.", ".ch/",
                   ".nl/", ".es/", ".fr/", ".zh/", ".sv/", ".pt/", ".no/", ".ja/", ".id/", ".fi/", ".de/", ".cs/",
                   ".au/", ".it/", ".hu/", ".da/", " "]
        if value.startswith("http") or value.startswith(
                "www") or ".org" in value or ".com" in value or ".net" in value or "encyclopediadramatica.wiki" in value:
            for el in exluded:
                if el in value:
                    return False
    ...
    refs_df = pd.DataFrame(data=refs2,
                           columns=["ref_wikipedia", "ref_urban_dict", "ref_encyc_dramatica", "ref_meme_generator",
                                    "ref_reddit", "ref_twitter", "ref_other"], index=df['url'].tolist())
    ...
    excluded_starts = ['uba3', 'ub4', 'uac', 'u9', 'u8', 'u7', 'u6', 'u5', 'u4', 'u3', 'u2', 'u1', 'u0', '/m/', '(u',
                       '#u', '^', '!1']
    ...
    excluded = ['4chan', 'youtube', 'image macro', 'twitter', 'meme', 'photoshop', 'tumblr', 'video', 'reddit',
                'facebook', 'youtube poop', 'hashtag', 'image', 'gif', 'racism', 'memes', 'sex', 'you', 'photo', 'of',
                'i', 'image macros', 'google', 'is', 'youtube', 'comment', 'comments']
    tags = df['tags'].tolist()
    tags2 = []
    ...
    origin = ['origin', 'origins', 'origin:', 'orgin', 'origin and character', 'origin and spread', 'the origin',
              'origins:', 'search for the origin', 'origins/early development', 'origin.', 'origins and spread']
    spread = ['spread', 'popularity', 'in popular culture', 'spread and popularity', 'popular culture',
              'popularity and spread', 'viral spread', 'spread & popularity',
              'mainstream coverage and declining popularity', 'impact', 'fandom', 'online presence']
    search_interest = ['search interest', 'search', 'search interests', 'trend', 'search trends',
                       'google insights for search', 'search trend', 'search analytics', 'search interest:',
                       'search insight', 'google search interest', 'google insight', 'google insights', 'insight',
                       'notable developments']
    similar_images = ['examples', 'various examples', 'notable examples', 'notable images', 'variations',
                      'notable developments', 'derivatives', 'derivative works',
                      'direct derivatives']  # (some are pics from other sites)
                        ...
                        if img_or_txt in row[key]:
                            ...
                            if img_or_txt == "text":
                                for text in keyword_texts:
                                    if len(re.findall("in progress", text.lower())) == 0 and len(
                                            re.findall("not available", text.lower())) == 0:
                                        texts += text + "\n"
                            else:
                                texts += keyword_texts
    ...
    df = df[['title', 'url', 'description', 'added', 'updated', 'year', 'status', 'search_keywords', 'type',
             'similar_images', 'has_relations', 'has_other_texts', 'has_refs', 'template_image_url', 'fb_image_url']]
