...
excluded = ['meme', 'YouTube', 'image macro', 'catchphrase', 'macros', 'Twitter', 'expression', '4chan', 'Tumblr', 'hashtag', 'remix', 'Internet', 'Reddit', 'social media', 'slang', 'English', 'Facebook', 'viral video', 'video sharing',  'fandom', 'copypasta', 'music video', 'phrasal template', 'Instagram', 'YTMND', 'YouTube Poop', 'GIF', 'Meme', 'imageboard', 'Internet slang', 'internet slang', 'YouTuber', 'Google', 'subreddit', 'Photoshop', 'MS Paint', 'Go', 'This Is', '/pol/', 'animated GIF', 'flash animation', 'internet meme', 'macro', 'internet memes',  'Youtube Poop', '2channel', 'fuck', 'stock photo', '3D', 'Windows', 'verb', '2chan', 'signify', 'GIF animation', 'Internet slang', 'internet slang']
...
stopwords = ['meme', 'image', 'internet', 'your', 'know', 'the',  'of', 'gif', 'video', 'and', 'photo', 'clip', 'font', 'caption', 'catchphrase', 'text', 'blog',  'macro', 'you', 'is',  'my', 'in', 'media', 'memes', 'to', 'youtube', 'on', 'for',  'me', 'it', 'with', 'jpeg', 'this',  'online', 'wiki', 'one',  'that', 'not', 'at', 'can', 'are', 'what', 'download', 'up', 'youtuber', 'do', 'its', 'all', 'dont', 'us', 'we', 'get', 'have', 'has', 'twitter', 'be', 'ii', 'from', 'am', 'will', 'de', 'out', 'how', 'facebook', 'pictures', 'just', 'an', 'iii', 'was', 'if', 'who', 'when', 'over', 'picture', 'by', 'down', 'cant']
...
df_main = df_main[['title', 'url', 'description', 'added', 'updated', 'year', 'has_relations', 'has_other_texts', 'has_refs', 'has_DBpedia', 'status', 'adult', 'medical',  'racy', 'spoof', 'violence', 'template_image_url', 'fb_image_url', 'alt_img_urls', 'similar_images', 'similar_images2', 'image_recogn_labels', 'image_recogn_scores', 'search_keywords']]
...
df_main.to_csv(path_to_cleaned_tmp + 'main_df.csv', sep=';', index=True, encoding='utf-8') 
refs_df.to_csv(path_to_cleaned_tmp + 'refs_df.csv', sep=';', index=True, encoding='utf-8') 
relations_df.to_csv(path_to_cleaned_tmp + 'relations_df.csv', sep=';', index=True, encoding='utf-8') 
textual_df.to_csv(path_to_cleaned_tmp + 'textual_df.csv', sep=';', index=True, encoding='utf-8') 
vision_annot_rev_df.to_csv(path_to_cleaned_tmp + 'vision_annot_df.csv', sep=';', index=True, encoding='utf-8') 
DBpedia_df.to_csv(path_to_cleaned_tmp + 'DBpedia_df.csv', sep=';', index=True, encoding='utf-8') 
...
relations_df.to_json(path_to_cleaned + 'relations.json', force_ascii=False, orient='index') # orient = 'columns'
# {meme_id: {'parent': 'shock-sites', 'children': [..], 'siblings': [...]}, 
   # ... }

# DBpedia_df:  'keywords', 'scores', 'links' (all lists)
DBpedia_df.to_json(path_to_cleaned + 'DBpedia.json', force_ascii=False, orient='index') 
# {meme_id: {'keywords': [...], 'scores': [...], 'links': [...]}, 
   # ... }

# vision_annot_rev_df:  'memes', 'scores' (all lists)
vision_annot_rev_df.to_json(path_to_cleaned + 'vision_annot.json', force_ascii=False, orient='index') 
...
df_similar_img_urls.to_json(path_to_cleaned + 'similar_img_urls.json', force_ascii=False, orient='index') 
# {meme_id: {'similar_images': [...], 'similar_images2': [...]}, 
   # ... }

df_image_recogn = df_main[['image_recogn_labels', 'image_recogn_scores']]
df_image_recogn.to_json(path_to_cleaned + 'image_recogn.json', force_ascii=False, orient='index') 
...
df_alternative_urls.to_json(path_to_cleaned + 'alternative_urls.json', force_ascii=False, orient='columns') #'index'                        

df_search = df_main[['search_keywords']]
df_search.to_json(path_to_cleaned + 'search_keywords.json', force_ascii=False, orient='columns') 
